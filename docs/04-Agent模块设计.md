...
当前实现中，该表同时承载 Agent 回复的 `sources` 和工具检索命中的 `retrievedMaterials`，用于记录本次回答过程中 `search_course_materials`、`read_course_material` 实际命中的资料列表。
...
系统在创建课程时写入默认 Agent 名称、提示词模板和检索范围。教师、学生和创建者均不在页面中编辑提示词。
...
Agent 运行时需要维护一个检索资料列表，用于存放资料检索工具命中的资料。该列表随单次回答返回为 `retrievedMaterials`，并在历史消息中继续返回，便于前端展示“本次回答检索过哪些资料”。
    ...
    "retrievedMaterials": [
      {
        "materialNodeId": 2,
        "fileName": "第一章课件.pdf",
        "snippetText": "操作系统是管理计算机硬件与软件资源的系统软件"
      }
    ],
    "sources": [
      {
